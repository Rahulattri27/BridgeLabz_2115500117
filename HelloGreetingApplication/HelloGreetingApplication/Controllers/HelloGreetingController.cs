...
public class HelloGreetingController : ControllerBase
{
    ...
    [HttpPost("AddNew")]
    public async Task<IActionResult> CreateGreeting(GreetingModel greeting)
    {
        _logger.LogInformation("Creating the greeting in database");
        var message = await _greetingBL.Create(greeting);
        var response = new ResponseModel<string>
        {
            Success = true,
    ...
    public async Task<IActionResult> GetGreetings()
    {
        ...
        List<GreetingModel> result = await _greetingBL.GetDatabaseGreeting();
